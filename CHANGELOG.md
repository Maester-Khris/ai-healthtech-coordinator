...
**Track B (DeepEval) real results — 2026-08-07, branch `feat/graphrag-eval-track-ab`, closing Task 6 of `docs/superpowers/plans/2026-08-05-v1-v2-retrieval-eval-fairness.md`.** Generated 20 real Track B transcripts via live Groq + Neo4j (`generate_track_b_transcripts.py`, no quota issues this run) and scored 18 complaint-grounded transcripts with DeepEval (`run_track_b_deepeval.py`): faithfulness mean 1.000 (100% pass), contextual precision mean 0.337 (33.3% pass), contextual recall mean 0.333 (27.8% pass). Reading: v2 retrieval is grounded (doesn't fabricate) but pulls incomplete/wrong context most of the time — consistent with Track A's hit-rate finding (see the 2026-08-18 correction below: 7/20, not the 8/20 quoted at the time this entry was written). Raw transcripts and results JSON are gitignored per repo convention (`backend/scripts/graphrag_eval/{transcripts,results}/`); these numbers are the durable record. Plan Tasks 1-8 all now complete on this branch.

**Task 13 completed — 2026-08-09, full 27-vignette × 2-leg (`off`/`neo4j`) run, 54/54 vignette-legs, branch `feat/symptom-understanding-v2-eval`.** Two more resume attempts were needed after the entry above: one hit a new, unrelated failure — `groq.BadRequestError: tool_use_failed` (the model occasionally emits a tool-call argument as the wrong JSON type, e.g. `"true"` instead of `true`), which crashed the whole process because nothing retried it — fixed with a single-retry wrapper around the Groq chat call in `backend/llm/groq_client.py` (transient sampling glitch, not a systematic prompt/schema problem; also benefits the real `/triage` endpoint, which had the same unguarded exposure). The next resume then hit the expected Groq quota wall again (Used 99798/100000) at 35/54 legs, but this time the error reported an exact reset window (~19 min, rolling window rather than a calendar-day reset) rather than requiring a full next-day wait; the final resume after that completed the remaining 19 legs cleanly.

Results (`results/ablation_results_20260809T023102Z.json`): confusion-matrix accuracy `off` 63.0% vs `neo4j` 66.7% (both legs' under-triage rate identical at 22.2%, n=27 each); mean elicitation coverage `off` 21.1% vs `neo4j` 24.3%. Information gain per turn was noisy and centered near zero for both legs (median 0, wide -0.03..+0.04 range) — not a clean differentiator at this sample size. Read together with the Track A/B numbers on `feat/graphrag-eval-track-ab`: v2 shows a small edge over v1 on this eval's own accuracy/coverage metrics despite v2's weak retrieval-precision/recall in that separate eval — the two eval efforts still haven't been reconciled into one story (see the "not yet reconciled" note above), and this is exactly the kind of apparent tension that reconciliation needs to explain, not paper over. **Update:** this tension is reconciled in the "Retrieval vs. Reasoning" case-study artifact (published 2026-08-13) — see the mechanism discussion after the symmetric Track A/B table below.
...
|---|---|---|
| Track A retrieval hit-rate | 100% (20/20) | ~~40% (8/20)~~ **see correction below** |
...
**Correction — Track A's 40% (8/20) v2 figure was stale, and the lay-phrasing scenario set was finally run — 2026-08-18.** The 8/20 number above (and everywhere else it's been quoted, including the published artifact) turned out to be the *pre-fix* number from the original Sprint 19 run — the Task 1/2 ranking fixes (`bd28c24`, `e3dc24f`, both 2026-08-05) were never actually validated by re-running Track A afterward; the only `track_a_results_*.json` on disk predated both fixes. Re-run today: **v2's real hit-rate is 7/20 (35%)**, and its recall rate is also 35% — identical to its hit-rate, meaning every miss is a coverage failure (the concept was never among the candidates), not a selection failure. The ranking fix had nothing to correct in this scenario set.

More importantly, `LAY_SCENARIOS` (Task 3's vocabulary-neutral scenario set, built 2026-08-05, never executed until today) was run for the first time: **both v1 and v2 scored 0/10 (0%)**. This isn't a bug — neither provider does semantic/vector retrieval in its live request path. v2's live Cypher (`queries.py`, `WHERE toLower($text) CONTAINS toLower(d.term)`) is literal substring matching against SNOMED description terms, the same class of mechanism as v1's alias substring match, just over a larger and more clinically formal vocabulary — which is if anything worse-suited to lay phrasing, not better. The plan's own architecture section describes v2 as "hybrid vector+Cypher retrieval"; no vector/embedding step exists anywhere in the live path (`anchor_mapping.py`'s "textual-similarity" mentions are from one-time offline KG-curation notes, not live retrieval). **Revised reading:** v1's near-perfect Track A score is a vocabulary-overlap artifact of the main scenario set, not evidence of a better retrieval mechanism — on language neither provider was calibrated to, both fail identically and completely. See `artifacts/2026-08-13-retrieval-vs-reasoning-v1-v2-eval-linkedin-handoff.md` (2026-08-18 revision) for the full write-up.
...
**`test_layer_isolation` gap closed — 2026-08-18.** The 2026-08-03 whole-branch review flagged a Phase 2 structural-protection test as never written: a guarantee that Layer 1 ingestion (`load_rf2.py`) can never clobber Layer 2 (`seed_red_flags.py`'s `RedFlag`/`FollowupQuestion`/`RedFlagCluster`). `seed_red_flags.py` already had the reverse-direction test (`test_seed_red_flags_never_writes_layer_1_labels`); the missing half is now `test_load_rf2.py::test_load_rf2_never_writes_layer_2_labels` — a source-regex check on `load()`'s own code (scoped there, not the whole module, so the module docstring's documented manual-wipe escape hatch doesn't false-positive), asserting no MERGE/CREATE/SET/constraint on the Layer 2 labels or their relationship types, and no `DELETE` at all. Positive control confirms it's not vacuous. 455/455 backend tests passing. This closes one item from the review's outstanding list; the rest (full `PART_OF`/cross-symptom-cluster authoring, expanding the entity-linking precision suite beyond 3 pilot anchors now that Neo4j is reachable, the remaining 7 Important findings) are explicitly deferred to a future symptom-understanding-improvement sprint (vocabulary-agnostic parsing + v2 graph construction), not this one.

**Two `/for-engineers` case studies drafted (not yet published) — 2026-08-29.** "Symptom-Understanding Retrieval: CTAS Lookup + SNOMED Knowledge Graph" (the v1/v2 architecture story) and "Fair Retrieval Evaluation: Static Lookup vs. SNOMED Knowledge Graph" (the eval-methodology story, including the corrected 7/20 Track A number and the LAY_SCENARIOS 0/10 result) added to `caseStudies.ts`, both tagged `#Draft` — this substantially delivers Sprint 19's originally-scoped "reasoning doc" and "case study" deliverables, but is marked draft rather than final pending a production-default confirmation (see below) and a decision on the deferred LinkedIn post. Publishing the retrieval-vs-reasoning numbers on LinkedIn (`artifacts/2026-08-13-retrieval-vs-reasoning-v1-v2-eval-linkedin-handoff.md`) was deliberately deferred, not cancelled — the numbers are now durably recorded in both the artifact and the two case studies, so the deferral costs nothing.
