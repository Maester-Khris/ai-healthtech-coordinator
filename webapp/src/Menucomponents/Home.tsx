...
import { Hospital, Person } from "@phosphor-icons/react";
...
const providerIcon = buildDivIcon(
    <Hospital size={32} weight="bold" style={{ color: "#dc2626" }} />
);
...
interface RouteData {
    ...
    // No routeGeometry from matrix API, so we'll draw straight lines
}
const waterBoxes = [
...
function generateRandomPointsInRadius(centerLat: number, centerLng: number, radiusKm: number, numPoints: number, maxAttemptsPerPoint: number = 50) {
    ...
    const isRoughlyOnLandToronto = (lat: number, lng: number): boolean => {
        // Broad check to exclude large parts of Lake Ontario south of the city
        if (lat < 43.59) {
            return false;
        }
        // if (lat < 43.640 && lng > -79.45 && lng < -79.33) {
        //     const isOverTorontoIslands = (currentLat: number, currentLng: number) => {
        //         const minLat = 43.614;
        //         const maxLat = 43.626;
        //         const minLng = -79.41;
        //         const maxLng = -79.34;
        //         return currentLat > minLat && currentLat < maxLat && currentLng > minLng && currentLng < maxLng;
        //     };
        //     if (!isOverTorontoIslands(lat, lng)) {
        //         return false;
        //     }
        // }
...
function Home() {
    ...
    //const peoplesPositions: Entity[] = generateRandomPointsInRadius(cnTowerPosition[0], cnTowerPosition[1], initial_lookup_radius, initial_people);
    ...
    // Effect to fetch routes from Geoapify
    useEffect(() => {
        const fetchRoutes = async () => {
            //print the correct information
            console.log(GEOAPIFY_API_KEY);
            console.log(peoplesPositions.length);
            console.log(healthProviders.length);
            console.log("fetch route started...");
            ...
            try {
                ...
                console.log("Geoapify Route Matrix Response:", result);
                console.log("fetch route started... stage 3 result parsed");
                ...
                console.error('Error fetching Geoapify Route Matrix:');
                console.error(error)
            } finally {
        ...
        fetchRoutes();
    }, [peoplesPositions, healthProviders, GEOAPIFY_API_KEY, formatTime]);
